...
class ShipmentTrackingByNote
{
    ...
    {
        $this->enabled      = ($this->get_option('wc_stbn_enabled') === 'yes');
        ...
        $args = array(
            ...
            'date_shipped'             => wc_clean( date('c') ), // ISO 1601 format for strtotime
     ...
     * This can be overridden by `woocommerce_stbn_shipping_carrier` hook
     * @since 1.1.0
     */
    private function get_carrier($note)
    {
        $default_carrier = $this->get_option('wc_stbn_shipping_carrier');
        $order_id = $note->comment_post_ID;

        return apply_filters('woocommerce_stbn_shipping_carrier', $default_carrier, $order_id);
    }
